...
namespace webapp.SOURCE
{
    ...
    abstract class User
    {
        ...
        public static bool Insert(string Columns, string Values)
        {
            ...
            if (Columns == null)
            {
                OleDbCommand cmd = new OleDbCommand("Insert Into [User] Values " + "(" + Values + ")", connection);
            }
            else
            {
                OleDbCommand cmd = new OleDbCommand("Insert Into [User]" + " (" + Columns + ") " + "Values" + " (" + Values + ")", connection);
            }
